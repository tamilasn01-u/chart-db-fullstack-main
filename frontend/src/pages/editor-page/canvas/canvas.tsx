...
export const Canvas: React.FC<CanvasProps> = ({ initialTables }) => {
    ...
    const CURSOR_THROTTLE_MS = 50; // Throttle WebSocket cursor updates to 20fps max
    ...
    const handleMouseMove = useCallback(
        (event: React.MouseEvent) => {
            ...
            if (
                ...
                sendCursorToWebSocket(position.x, position.y);
                lastCursorSentRef.current = now;
            }

            // Handle floating edge cursor position
            if (tempFloatingEdge) {
